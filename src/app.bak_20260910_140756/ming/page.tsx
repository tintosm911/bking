    ...
    <div className="min-h-screen bg-dark-900 text-white">
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 mt-2">八字用神起名 · 五格数理 · 五行补益，一名字定乾坤</p>
        </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setBirth({ ...birth, gender: 1 })}
                    className={`px-3 py-2 rounded-lg text-center text-sm font-semibold border transition ${
                      birth.gender === 1 ? "bg-gold text-dark-900 border-gold" : "bg-dark-700 border-gold/20 text-gray-400"
                    }`}>男</button>
                  <button type="button" onClick={() => setBirth({ ...birth, gender: 2 })}
                    className={`px-3 py-2 rounded-lg text-center text-sm font-semibold border transition ${
                      birth.gender === 2 ? "bg-gold text-dark-900 border-gold" : "bg-dark-700 border-gold/20 text-gray-400"
                    }`}>女</button>
                </div>
              </div>
            ...
            {result && (
              <div className="space-y-5">
                <div className="glass rounded-2xl p-5 border-gold/30">
                  ...
                  <p className="text-sm text-gray-300 leading-relaxed">{result.yongshenNote}</p>
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {(list as MingResult["single"]).slice(0, 9).map((s, i) => (
                        <div key={i} className="bg-dark-700/60 rounded-xl p-4 border border-gold/10">
                          ...
                          <div className="text-2xl font-serif font-bold text-white my-1">{s.score}<span className="text-sm text-gray-500">分</span></div>
                          <ScoreBar score={s.score} />
                          <p className="text-xs text-gray-400 mt-2 leading-relaxed">{s.meaning}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ))}
                <p className="text-xs text-gray-500 text-center">※ 起名参考传统文化姓名学，非科学依据；好名字更要重善、重愿、重德。</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            {score && (
              <div className="space-y-5">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {score.chars.map((c, i) => (
                      <span key={i} className="px-3 py-1.5 rounded-xl bg-dark-700/60 border border-gold/10 text-sm">
                        ...
                        {c.element ? <span className={`ml-2 px-1.5 rounded border text-xs ${EL_COLOR[c.element]}`}>{c.element}</span> : <span className="ml-2 text-gray-500 text-xs">中性</span>}
                      </span>
                    ))}
                  </div>
                ...
                <p className="text-xs text-gray-500 text-center">※ 数理吉凶为姓名学传统流派解读，供参详娱乐，非科学预测。</p>
              </div>
            )}
          </div>
        )}
