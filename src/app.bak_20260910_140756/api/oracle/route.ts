...
export async function POST(request: NextRequest) {
  try {
    ...
    // 调用玄机大师引擎（传入会话历史以支持追问接续）
    const reply = runOracle(text, session.profile, session.messages);

    // 有 DeepSeek key 时做 LLM 增强解读（无 key 自动降级，原样返回）
    let message = reply.message;
    try {
      const deepened = await deepenOracleReply(reply.message, reply.skill, reply.profile, session.messages.slice(-4).map((m) => ({ role: m.role, content: m.content })));
